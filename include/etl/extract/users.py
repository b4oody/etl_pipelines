...
def extract_users(timeout=30, retries=3):
    """
    Витяг користувачів з API
    ...
    for attempt in range(retries):
        try:
            logger.info(f"Extracting users (attempt {attempt + 1}/{retries})...")
            ...
            data = response.json()
            logger.info(f"[OK] Extracted {len(data)} users")
